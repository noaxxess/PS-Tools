...
.PARAMETER RightLogo
    ...
    -RightLogo "https://alwaysuptech.com/assets/img/ALlogo.png"
...
$ModHTML = Get-Module -ListAvailable -Name "ReportHTML"
...
#Check for GPO Module
$ModGPO = Get-Module -ListAvailable -Name "GroupPolicy"

If ($null -eq $ModGPO)
{
	
	Write-Host "Group Policy Management is not present, attempting to install it"
...
foreach ($DomainAdminMember in $DomainAdminMembers)
{
	...
	$LastBadPass = $AllUsers | Where-Object { $_Name -eq $Name} | Select LastBadPasswordAttempt
...
$Year = (Get-Date).Year
$ReportName = ("ADReport-$Day $Month $Year")
